function [forcex,forcey]=getLiftForceO4_AR8(x,y,rsph,U,rho,L)
%% INPUT:
%  Dimensions -4L < x < 4L, -.5L < y < .5L
...
%
%% DEPENDENCIES
%  Need to have HoLeal_channel_aspectratio8_Re1_mesh8_05-12-2017.mat in the
%  Matlab path.  Uses Matlab's interp2 for 2D interpolation
%
%% NOTES
...
AR = 8;
diffx = .5*AR*L-abs(x); diffy = .5*L-abs(y);

% if the location (x,y) is outside the channel, return an error
if x<-.5*AR*L || x>.5*AR*L || y<-L/2 || y>L/2
    error(['location outside channel: pick ',num2str(-.5*AR*L),' < x < ',num2str(.5*AR*L),' and ',num2str(-L/2),' < y < ',num2str(L/2)])
...
% otherwise proceed
    ...
    load HoLeal_channel_aspectratio8_Re1_mesh8_05-12-2017.mat
