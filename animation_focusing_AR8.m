...
%
%% AUTHOR
%  Kaitlyn Hood 2017 - kaitlyn.t.hood@gmail.com
%%%

% parameters:
Re = 36;                    % Reynolds number
W = 360e-6; H = 45e-6;       % height and width of channel (m)
...
m1=24; dx=(AR*L-2*alpha-.1*L)/(m1-1);
...
for j=1:size(xs,1)
    for l=1:size(xs,2)
        [forcex(j,l),forcey(j,l)]=getLiftForce_AR8(xs(j,l),ys(j,l),alpha,U,rho,L);
    end
end
...
%         try
%             drag = 6*pi*mu*alpha;
%             [utemp,vtemp] = getLiftForce_AR4(x(k,j-1),y(k,j-1),alpha,U,rho,L);
%             V = [utemp; vtemp]/drag;
%             [v1,v2] = getLiftImage_AR4(x(k,j-1),y(k,j-1),alpha,L);
%             v1 = alpha*v1*abs(V(1)); 
%             v2 = alpha*v2*abs(V(2));
%             vel = V + v1 + v2;
%             u(k) = vel(1); 
...
% Advect particles using the lift force
% fast method
load inertial_constants_AR8_n801_05-12-2017.mat
x=zeros(m,N); y=zeros(m,N);
u =zeros(m,1); v=zeros(m,1);
x(:,1)=r(:,1); y(:,1)=r(:,2); 
for j=2:N;
...
figure('Units','pixels','Position',[100,100,1600,275])
clf
