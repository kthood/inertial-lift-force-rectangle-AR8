function [forcex,forcey]=getLiftForce_AR8(x,y,rsph,U, rho, L)
%% INPUT:
...
%
%% DEPENDENCIES
%  Need to have HoLeal_channel_aspectratio8_Re1_mesh8_05-12-2017.mat in the Matlab path
...
if x<-.5*AR*L || x>.5*AR*L || y<-L/2 || y>L/2
    error(['location outside channel: pick ',num2str(-.5*AR*L),' < x < ',...
        num2str(.5*AR*L),' and ',num2str(-L/2),' < y < ',num2str(L/2)])
